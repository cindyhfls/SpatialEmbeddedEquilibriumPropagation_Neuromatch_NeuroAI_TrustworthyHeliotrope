...
def get_random_sample_dataloader(dataset:torch.utils.data.Dataset, batch_size:int, M:int):
    ...
    Get a DataLoader for a subset of size **M** per class from **dataset** with batch size **batch_size**.
    ...
    Returns:
        (torch.utils.data.DataLoader): DataLoader for a subset of size **M** per class from **dataset** with batch size **batch_size**
    """
...
def get_random_sample_train_val(train_dataset:torch.utils.data.Dataset, val_dataset:torch.utils.data.Dataset, batch_size:int, N_train_data:int):
    ...
    Args:
        ...
        batch_size (int): Batch size
        N_train_data (int): Number of samples per class in the train subset (~1/9 the number of samples per class in the validation subset, up to 30 for the validation subset)

    Returns:
        (torch.utils.data.DataLoader): train DataLoader for a subset of size **N_train_data** per class from **train_dataset** with batch size **batch_size**
        (torch.utils.data.DataLoader): DataLoader for a subset of size approximately **N_train_data**/9 (up to 30 max) per class from **val_dataset** with batch size **batch_size**
    """
